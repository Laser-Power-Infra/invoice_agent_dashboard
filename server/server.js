...
// MySQL connection pool configuration for high concurrency
const dbConfig = {
...
function sanitizeRows(rows) {
  return rows.map(r => {
    ...
    if (row.array) {
      try {
        const arr = typeof row.array === 'string' ? JSON.parse(row.array) : row.array;
        const jsonDate = arr.invoice_date || arr.party_inv_date;
        if (jsonDate && typeof jsonDate === 'string' && !jsonDate.includes('T')) {
          row.party_inv_date = jsonDate;
        }
      } catch (e) {}
    }
    return row;
...
async function ensureTableExists(conn, tableName, columns) {
  ...
  // Add search column indexes to boost multi-user query speed
  try {
...
// POST /api/update-record — proxy record updates back to Google Sheets database & MySQL
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (jsonResult) {
          console.log('Apps Script update response:', jsonResult);
          scriptResult = jsonResult;
        }
      }
    } catch (scriptErr) {
