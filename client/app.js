...
const SERVER_BASE_URL = import.meta.env.VITE_SERVER_BASE_URL || (
  (window.location.port === '6004' || window.location.port === '3000' || !window.location.port || window.location.hostname.includes('ngrok'))
    ? ''
    : `http://${window.location.hostname}:6005`
);
console.log('SERVER_BASE_URL resolved to:', SERVER_BASE_URL || '(relative /)');

// Helper fetch wrapper to bypass ngrok free warning interstitial pages
...
function getCurrentUserRole() {
  ...
  try {
    ...
    return data.role || (data.username && data.username.toLowerCase() === 'admin' ? 'admin' : 'user');
  } catch {
    return 'user';
  }
}
...
function applyRolePermissions() {
  ...
  // 1. Hide/Show the 3 top header buttons (#sync-data-btn, #repair-arrays-btn, #add-record-btn)
  ...
  if (repairBtn) repairBtn.style.display = isAdmin ? 'inline-flex' : 'none';
  if (addBtn) addBtn.style.display = isAdmin ? 'inline-flex' : 'none';

  // 2. Hide/Show Settings toggle button in sidebar footer
  ...
  // 3. Hide/Show Settings view tab and redirect normal users if currently on settings
    ...
    if (!isAdmin) {
      ...
      if (state.activeTab === 'settings') {
        ...
        document.querySelectorAll('.menu-item').forEach(i => {
          ...
          if (i.dataset.tab === 'dashboard') i.classList.add('active');
        });
      }
    } else {
  ...
  // 4. Any generic .admin-only elements
...
function initAuthActions() {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', (e) => {
      ...
      if (password === STATIC_PASSWORD || password === 'user123') {
        ...
        // Apply role permissions to hide/show buttons and settings
        applyRolePermissions();

        // Sync API and render dashboard views
        syncWithAPI(false);
        renderAllViews();
        showToast(`Login successful! Welcome (${userRole === 'admin' ? 'Admin' : 'User'}).`, 'success');
...
async function fetchAndRefresh(forceRefresh = false) {
  ...
  try {
    ...
    if (data && data.success) {
      const hasData = (data.invoices && data.invoices.length > 0) || (data.purchases && data.purchases.length > 0);
      if (hasData || !state.invoices.length) {
        ...
        console.warn('Received empty database payload from server; skipping state overwrite to preserve UI data.');
      }
    ...
    if (forceRefresh || (!state.invoices.length && !state.purchases.length)) {
      showToast('Server unreachable — showing cached state', 'warning');
    }
    renderAllViews();
  }
}
...
function loadLocalDatabase() {
  ...
  // Clean up legacy raw cache keys to free up LocalStorage quota
  localStorage.removeItem('db_raw_invoices');
  localStorage.removeItem('db_raw_purchases');
...
function saveToLocalStorage() {
  try {
    ...
    // Graceful silent handle for browser storage limits
  }
}
...
function saveLocalOverrides() {
  ...
  state.invoices.forEach(inv => {
    ...
    const fields = ['buyer_name','transporter_name','transporter_gstin','to_place_name','item_name',
      ...
      'net_payable','total_invoice_value','RCM','validated','validation_timestamp','ai_summary'];
...
// Convert any date string (DD-MM-YYYY or DD/MM/YYYY or ISO timestamp) to ISO yyyy-MM-dd for date inputs
function normalizeToISODate(val) {
  if (!val) return '';
  let s = String(val).trim();
  
  // If it's a full ISO timestamp with time (e.g. contains T or space with time), parse as local date
  if (s.includes('T') || (s.includes(' ') && s.match(/\d{2}:\d{2}/))) {
    const d = new Date(s);
    if (!isNaN(d.getTime())) {
  ...
  if (/^\d{4}-\d{2}-\d{2}/.test(s)) {
    let dateStr = s.substring(0, 10);
    const parts = dateStr.split('-');
    const year = parseInt(parts[0], 10);
    const month = parseInt(parts[1], 10);
    const day = parseInt(parts[2], 10);
    
    // Scenario A: Parsed date is in Jan-Mar 2026 (outside fiscal range), but original was Apr-Dec 2026 (day swapped into month)
    if (year === 2026 && month <= 3 && day >= 4 && day <= 12) {
      const correctedMonth = String(day).padStart(2, '0');
      const correctedDay = String(month).padStart(2, '0');
      dateStr = `${year}-${correctedMonth}-${correctedDay}`;
    }
    // Scenario B: Parsed date is in Apr-Dec 2027 (outside fiscal range), but original was Jan-Mar 2027 (day swapped into month)
    else if (year === 2027 && month >= 4 && day >= 1 && day <= 3) {
      const correctedMonth = String(day).padStart(2, '0');
      const correctedDay = String(month).padStart(2, '0');
      dateStr = `${year}-${correctedMonth}-${correctedDay}`;
    }
    // Scenario C: Google Sheets US locale converted DD-08-2026 (August dates like 10-08-2026) to 2026-MM-08 (e.g. 2026-10-08)
    else if (year === 2026 && day === 8 && month >= 4 && month !== 8 && month <= 12) {
      const correctedMonth = '08';
      const correctedDay = String(month).padStart(2, '0');
      dateStr = `${year}-${correctedMonth}-${correctedDay}`;
    }
    return dateStr;
...
function processRawData(data) {
  ...
  state.rawPurchases = data.purchases || [];
  ...
  state.invoices = rawInvoices.map((inv, idx) => {
    ...
    return {
      ...
      ai_summary: parsedArray.ai_summary || inv["AI SUMMRY"] || inv.ai_summary || ''
    };
  });
  ...
  state.purchases = rawPurchases.map((pur, idx) => {
    ...
    return {
      ...
      party_inv_date: (() => {
        const rawPurDate = parsedArray.invoice_date || parsedArray.party_inv_date || (pur.party_inv_date ? pur.party_inv_date.split('T')[0] : '');
        let pDate = normalizeToISODate(rawPurDate);
      ...
      ai_summary: pur["AI SUMMRY"] || pur.ai_summary || parsedArray.ai_summary || '',
  ...
  // Restore manually-validated flags & local overrides from local cache (survives server refresh)
  try {
...
function openFloatingPdfPanel(invNo) {
  ...
  if (rawUrl && (rawUrl.includes('drive.google.com') || rawUrl.includes('docs.google.com') || rawUrl.startsWith('http') || rawUrl.startsWith('blob'))) {
    ...
    body.innerHTML = `
      <div style="position:absolute;top:6px;right:6px;z-index:10;background:rgba(0,0,0,0.75);padding:4px 10px;border-radius:6px;backdrop-filter:blur(4px);">
        <a href="${escapeHtml(rawUrl)}" target="_blank" style="color:#fff;font-size:0.75rem;text-decoration:none;font-weight:600;display:inline-flex;align-items:center;gap:4px;"><i data-lucide="external-link" style="width:12px;height:12px;"></i> Open Direct Link</a>
      </div>
      <iframe src="${escapeHtml(embedUrl)}" style="width:100%;height:100%;border:none;display:block;" allow="autoplay; encrypted-media; fullscreen"></iframe>
    `;
  } else {
    body.innerHTML = `
...
// Helper to resolve invoice value dynamically (Freight + ST + GST if non-RCM)
function getResolvedInvoiceValue(inv, pur, lineItemsList) {
  const freight = (lineItemsList && lineItemsList.length > 0) 
                  ? lineItemsList.reduce((sum, item) => sum + (Number(item.freight) || 0), 0)
                  : (Number(pur?.bill_freight_val) || Number(inv?.bill_freight_val) || 0);
  const st = Number(pur?.st_charges) || Number(inv?.st_charges) || 0;

  const cgst = Number(pur?.cgst) || Number(inv?.cgst) || 0;
  const sgst = Number(pur?.sgst) || Number(inv?.sgst) || 0;
  const igst = Number(pur?.igst) || Number(inv?.igst) || 0;
  const purGst = Number(pur?.total_gst_value) || 0;
  const totalGst = cgst + sgst + igst + purGst;

  const gstPercent = freight > 0 ? (totalGst / freight) : 0;

  const rcmVal = (pur && pur.rcm) || (inv && inv.RCM) || 0;
  const rcmStr = String(rcmVal);
  const isRcm = rcmVal > 0 || rcmStr.includes('5%') || (gstPercent > 0 && gstPercent <= 0.055) || totalGst <= 12;

  if (isRcm) {
    // Under RCM (5% GST or 0%), GST is paid directly by buyer, so Vendor Invoice Value = Freight + ST Charges
    const baseVal = freight + st;
    if (baseVal > 0) return baseVal;
    return Number(pur?.net_payable) || Number(inv?.net_payable) || Number(pur?.total_invoice_value) || Number(inv?.total_invoice_value) || 0;
  } else {
    // For Forward Charge GST (12% / 18%), Vendor Invoice Value includes full GST
    const baseVal = freight + st + totalGst;
    if (baseVal > 0) return baseVal;
    return Number(pur?.total_invoice_value) || Number(inv?.total_invoice_value) || Number(pur?.net_payable) || Number(inv?.net_payable) || 0;
  }
}
window.getResolvedInvoiceValue = getResolvedInvoiceValue;

async function handleReprocessPDF(pdfUrl, invoiceNumber, billType) {
  const btn = document.getElementById(billType === 'purchase' ? 'btn-reprocess-purchase-pdf' : 'btn-reprocess-invoice-pdf');
  if (!btn) return;
  
  const originalText = btn.innerHTML;
  btn.disabled = true;
  btn.innerHTML = `<i class="animate-spin" data-lucide="loader-2" style="width: 12px; height: 12px; display: inline-block;"></i> Reprocessing...`;
  if (typeof lucide !== 'undefined') lucide.createIcons();

  try {
    showToast("Duplicate copy triggers started. Processing PDF copy in background...", "info");
    const resp = await fetch(`${SERVER_BASE_URL}/api/reupload`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ fileUrl: pdfUrl, invoiceNumber, billType })
    });
    
    if (!resp.ok) {
      const err = await resp.json();
      throw new Error(err.error || 'Server error during reprocess');
    }
    
    const data = await resp.json();
    if (data.success && data.fileUrl) {
      showToast("PDF duplicated & re-uploaded successfully! AI extraction started in background.", "success");
      
      // Update local state with the new copy URL
      if (billType === 'purchase') {
        state.purchases = state.purchases.map(p => {
          if (p.party_inv_no === invoiceNumber) p.pdf_url = data.fileUrl;
          return p;
        });
      } else {
        state.invoices = state.invoices.map(i => {
          if (i.invoice_number === invoiceNumber) i.pdf_url = data.fileUrl;
          return i;
        });
      }
      ...
      // Send Sheet update writeback
      if (billType === 'purchase') {
        sendSheetUpdate('purchase', invoiceNumber, { pdf_url: data.fileUrl });
      } else {
        sendSheetUpdate('invoice', invoiceNumber, { pdf_url: data.fileUrl });
      }
      
      // Reload current modal after a slight delay to show updated PDF container URL
      setTimeout(() => {
        openDetailedRecordModal(invoiceNumber);
      }, 1000);
    } else {
      throw new Error("Invalid server response parameters");
    }
  } catch (e) {
...
window.handleReprocessPDF = handleReprocessPDF;

async function reviewActiveBill() {
  const selectedId = state.selectedRecordId;
  
  if (!selectedId) {
    showToast("No bill selected to mark for review", "error");
    return;
  }

  const remark = prompt("Please enter a remark/reason for Dev Review (optional):");
  if (remark === null) {
  ...
  const cleanId = String(selectedId).trim().toLowerCase();
  ...
  if (typeof selectedId === 'string' && (selectedId.startsWith('inv-') || selectedId.startsWith('pur-'))) {
    if (selectedId.startsWith('inv-')) {
      ...
      if (invoice) {
        ...
        purchase = [...state.purchases].reverse().find(p => String(p.party_inv_no).trim().toLowerCase() === cleanInvNo && p.bill_freight_val === invoice.bill_freight_val)
                || [...state.purchases].reverse().find(p => String(p.party_inv_no).trim().toLowerCase() === cleanInvNo) || null;
      }
    } else {
      purchase = state.purchases.find(p => p.id === selectedId) || null;
      if (purchase) {
        invoiceNumber = purchase.party_inv_no;
        const cleanPurNo = String(invoiceNumber).trim().toLowerCase();
        invoice = [...state.invoices].reverse().find(i => String(i.invoice_number).trim().toLowerCase() === cleanPurNo && i.bill_freight_val === purchase.bill_freight_val)
               || [...state.invoices].reverse().find(i => String(i.invoice_number).trim().toLowerCase() === cleanPurNo) || null;
      }
    }
  } else {
    invoiceNumber = selectedId;
    const cleanNo = String(invoiceNumber).trim().toLowerCase();
    invoice = [...state.invoices].reverse().find(i => String(i.invoice_number).trim().toLowerCase() === cleanNo) || null;
    purchase = [...state.purchases].reverse().find(p => String(p.party_inv_no).trim().toLowerCase() === cleanNo) || null;
  }

  let label = "Dev Review need";
  if (remark.trim() !== '') {
    label = `Dev Review need: ${remark.trim()}`;
  }

  // Update locally
  if (purchase) {
    let summary = purchase.ai_summary || '';
    // Strip any existing Dev Review need line (case insensitive)
    ...
    purchase.ai_summary = summary;
    purchase._manually_edited = true;

    if (purchase._rawArray) {
      try {
        const arrObj = JSON.parse(purchase._rawArray);
        arrObj.ai_summary = summary;
        purchase._rawArray = JSON.stringify(arrObj);
      } catch (e) {}
    }
  }
  
  if (invoice) {
    let summary = invoice.ai_summary || '';
    summary = summary.replace(/\n*Dev\s+Review\s+need[^\n]*/gi, '');
    const match = summary.match(/([\s\S]*?)(\n*={10,}\s*)$/);
    if (match) {
      summary = match[1].trim() + "\n\n" + label + "\n\n" + match[2].trim() + "\n";
    ...
    invoice._manually_edited = true;

    if (invoice._rawArray) {
      try {
        const arrObj = JSON.parse(invoice._rawArray);
        arrObj.ai_summary = summary;
        invoice._rawArray = JSON.stringify(arrObj);
      } catch (e) {}
    }
  }
  ...
  showToast(`✓ Bill ${invoiceNumber} marked for Dev Review!`, "success");

  // Push updates to Google Sheets & MySQL database
  if (purchase) {
    sendSheetUpdate('purchase', purchase.party_inv_no, { ai_summary: purchase.ai_summary }, purchase);
  }
  if (invoice) {
    sendSheetUpdate('invoice', invoice.invoice_number, { ai_summary: invoice.ai_summary }, invoice);
  }

  // Update modal header status badge to show Dev Review Need in yellow
  const statusBadge = document.getElementById('modal-badge-status');
  if (statusBadge) {
    statusBadge.textContent = 'Dev Review Need';
  ...
  // Refresh details pane summary text
  const currentSummary = (purchase && purchase.ai_summary) || (invoice && invoice.ai_summary) || '';
  const summaryView = document.getElementById('modal-ai-summary-view');
  if (summaryView) {
    summaryView.innerHTML = renderRichAiSummary(currentSummary);
  }
  const editArea = document.querySelector('textarea[name="pur_ai_summary"]');
  if (editArea) {
    editArea.value = currentSummary;
  }
}
window.reviewActiveBill = reviewActiveBill;
...
function generateMergedERPRows(invoiceNumber) {
  ...
  const taggingRowsData = dedupedLineItems.map((item) => {
    let itemFreight = Number(item.freight) || 0;
    const foRate = parseNumericValue((inv && inv.fo_rate) ?? (pur && pur.fo_rate) ?? 0);
    const foQty = parseNumericValue((inv && inv.fo_qty) ?? (pur && pur.fo_qty) ?? 1);
    const foOrderVal = parseNumericValue((inv && inv.fo_order_value) ?? (pur && pur.fo_order_value) ?? (foRate * foQty));

    if (foRate > 0 && foQty > 1 && itemFreight === foOrderVal) {
      itemFreight = foRate;
    }
    const st = Number((pur && pur.st_charges) || (inv && inv.st_charges) || item.st_charges || 0);
    ...
    return {
      ...
      "FREIGHT VALUE": itemFreight,
      "ST CHARGES": st,
      "Invoice Value (₹)": getResolvedInvoiceValue(inv, pur, [{ ...item, freight: itemFreight }])
    };
  });
...
function isRecordRightBill(type, record) {
  let invoice = null;
  let purchase = null;
  ...
  if (type === 'invoice') {
    ...
    purchase = [...state.purchases].reverse().find(p => 
      String(p.party_inv_no).trim().toLowerCase() === String(invoice.invoice_number).trim().toLowerCase() && 
      p.bill_freight_val === invoice.bill_freight_val
    ) || [...state.purchases].reverse().find(p => 
      String(p.party_inv_no).trim().toLowerCase() === String(invoice.invoice_number).trim().toLowerCase()
    ) || null;
  } else {
    purchase = record;
    // Find matching invoice
    invoice = [...state.invoices].reverse().find(i => 
      String(i.invoice_number).trim().toLowerCase() === String(purchase.party_inv_no).trim().toLowerCase() && 
      i.bill_freight_val === purchase.bill_freight_val
    ) || [...state.invoices].reverse().find(i => 
      String(i.invoice_number).trim().toLowerCase() === String(purchase.party_inv_no).trim().toLowerCase()
    ) || null;
  }

  // 1. Check Dev Review Need (should skip if present in either invoice or purchase)
  const isDevReview = (purchase?.ai_summary?.toLowerCase().includes('dev review need')) || 
                      (invoice?.ai_summary?.toLowerCase().includes('dev review need'));
  if (isDevReview) return false;

  // 2. Check if manually validated (always counts as passed/right if not marked as Dev Review need)
  const isManuallyValidated = (invoice?.validated || purchase?.validated);
  if (isManuallyValidated) return true;

  // 3. Check for validation failures (skip if there are failures)
  const isWrong = purchase ? hasValidationFailures(purchase) : false;
  
  // 4. Check for missing critical tracking fields (skip if missing)
  const isFoOrInvMissing = (invoice && ((!invoice.fo_no || invoice.fo_no === '-') || (!invoice.invoice_number || invoice.invoice_number === '-')))
                        || (purchase && ((!purchase.fo_no || purchase.fo_no === '-') || (!purchase.party_inv_no || purchase.party_inv_no === '-')));

  if (isWrong || isFoOrInvMissing) {
    return false; // Skip if wrong or missing critical fields
  }

  return true; // Correct / right bill!
}
...
function exportRange(type, startDate, endDate) {
  ...
  const matching = pool.filter(item => {
    ...
    return itemInt >= startInt && itemInt <= endInt && isRecordRightBill(type, item);
  });
  if (matching.length === 0) {
    showToast('No correct/right records found within this range.', 'warning');
    return;
  }
...
async function pollAgentNotification() {
  try {
    const resp = await safeFetch(`${SERVER_BASE_URL}/api/agent-notification?_=${Date.now()}`);
    if (!resp.ok) return;
    const data = await resp.json();
...
function renderInvoicesLedger() {
  ...
  sortedFiltered.forEach(inv => {
    ...
    if (currentDate !== lastDate) {
      ...
      if (exportBtn) {
        exportBtn.addEventListener('click', (e) => {
          ...
          }).filter(item => isRecordRightBill('invoice', item));
    ...
    const isDevReview = (matchingPur?.ai_summary?.toLowerCase().includes('dev review need')) || 
                        (inv.ai_summary?.toLowerCase().includes('dev review need'));
    ...
    const alertIcon = isDevReview ?
      `<span style="margin-left: 6px; color: #ffc107; vertical-align: middle;" title="Dev Review Need"><i data-lucide="help-circle" style="width: 14px; height: 14px; display: inline-block;"></i></span>` :
      (isRowAlert ? `<span style="margin-left: 6px; color: var(--accent-red); vertical-align: middle;" title="${escapeHtml(alertMsg)}"><i data-lucide="alert-triangle" style="width: 14px; height: 14px; display: inline-block;"></i></span>` : '');
...
function renderPurchasesLedger() {
  ...
  sortedFiltered.forEach(pur => {
      ...
      if (exportBtn) {
        exportBtn.addEventListener('click', (e) => {
          ...
          const dateRows = filtered.filter(item => {
            const rawDate = item.upload_date || '';
            return (rawDate ? formatToDDMMYYYY(rawDate) : 'No Upload Date') === targetDate;
          }).filter(item => isRecordRightBill('purchase', item));
    ...
    const isDevReview = pur.ai_summary?.toLowerCase().includes('dev review need');
    tr.className = 'clickable-row ' + groupClass;
    if (isDevReview) {
      tr.style.backgroundColor = 'rgba(255, 193, 7, 0.08)';
      tr.style.borderLeft = '4px solid #ffc107';
      tr.title = "Dev Review Need";
    } else if (isWrong && !(pur.validated)) {
      tr.style.backgroundColor = 'var(--accent-red-glow)';
      tr.style.borderLeft = '4px solid var(--accent-red)';
    }
...
function renderReconciliation() {
  ...
  filtered.forEach(r => {
    ...
    let statusBadge = '';
    const isDevReview = (r.purchase?.ai_summary?.toLowerCase().includes('dev review need')) || 
                        (r.invoice?.ai_summary?.toLowerCase().includes('dev review need'));
    if (isDevReview) statusBadge = '<span class="badge badge-warning" style="background-color: #ffc107; color: #000; border: none;">Dev Review Need</span>';
    else if (r.status === 'MATCH') statusBadge = '<span class="badge badge-green">Matched</span>';
...
function openDetailedRecordModal(target) {
  ...
  let purchase = null;
  let invoiceNumber = '';
  let selectedId = String(target || '').trim();

  // Try to find by unique ID first to keep precise row selection on duplicates/splits
  if (selectedId.startsWith('inv-') || selectedId.startsWith('pur-')) {
    if (selectedId.startsWith('inv-')) {
      invoice = state.invoices.find(i => i.id === selectedId) || null;
      if (invoice) {
        invoiceNumber = invoice.invoice_number;
        const cleanInvNo = String(invoiceNumber).trim().toLowerCase();
        purchase = [...state.purchases].reverse().find(p => 
          String(p.party_inv_no).trim().toLowerCase() === cleanInvNo && 
          p.bill_freight_val === invoice.bill_freight_val && 
          String(p.lorry_vehicle_no || '').trim().toLowerCase() === String(invoice.lorry_vehicle_no || '').trim().toLowerCase()
        ) || [...state.purchases].reverse().find(p => 
          String(p.party_inv_no).trim().toLowerCase() === cleanInvNo && 
          p.bill_freight_val === invoice.bill_freight_val
        ) || [...state.purchases].reverse().find(p => 
      ...
      if (purchase) {
        ...
        const cleanPurNo = String(invoiceNumber).trim().toLowerCase();
        invoice = [...state.invoices].reverse().find(i => 
          String(i.invoice_number).trim().toLowerCase() === cleanPurNo && 
          i.bill_freight_val === purchase.bill_freight_val && 
          String(i.lorry_vehicle_no || '').trim().toLowerCase() === String(purchase.lorry_vehicle_no || '').trim().toLowerCase()
        ) || [...state.invoices].reverse().find(i => 
          String(i.invoice_number).trim().toLowerCase() === cleanPurNo && 
          i.bill_freight_val === purchase.bill_freight_val
        ) || [...state.invoices].reverse().find(i => 
  ...
  // Fallback if not found by ID or if target is a raw invoice number
  if (!invoice && !purchase) {
    invoiceNumber = selectedId.replace(/^(inv|pur)-/, '').replace(/_/g, '/');
    const cleanNo = String(invoiceNumber).trim().toLowerCase();
    invoice = [...state.invoices].reverse().find(i => String(i.invoice_number).trim().toLowerCase() === cleanNo) || null;
    purchase = [...state.purchases].reverse().find(p => String(p.party_inv_no).trim().toLowerCase() === cleanNo) || null;
    if (purchase) selectedId = purchase.id;
    else if (invoice) selectedId = invoice.id;
  }

  // Store selected unique ID (critical for updating the correct row on duplicate bills)
  state.selectedRecordId = selectedId;
      ...
      pdfContainer.innerHTML = `
        ...
        <div class="pdf-attachment-card">
          ...
          <div class="pdf-attachment-actions" style="display: flex; gap: 8px;">
            ...
            <button class="btn btn-purple btn-xs" id="btn-reprocess-invoice-pdf" style="background-color: var(--accent-purple); color: white; border: none; display: flex; align-items: center; gap: 4px;"><i data-lucide="refresh-cw"></i> Reprocess</button>
            <button class="btn btn-danger btn-xs" id="btn-delete-pdf-attachment"><i data-lucide="trash-2"></i> Unlink</button>
          </div>
        </div>
      `;
      
      // Add reprocess listener
      const reprocessBtn = document.getElementById('btn-reprocess-invoice-pdf');
      if (reprocessBtn) {
        reprocessBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          handleReprocessPDF(invoice.pdf_url, invoice.invoice_number, 'invoice');
        });
      }
      ...
      purchasePdfContainer.innerHTML = `
        ...
        <div class="pdf-attachment-card">
          ...
          <div class="pdf-attachment-actions" style="display: flex; gap: 8px;">
            ...
            <button class="btn btn-purple btn-xs" id="btn-reprocess-purchase-pdf" style="background-color: var(--accent-purple); color: white; border: none; display: flex; align-items: center; gap: 4px;"><i data-lucide="refresh-cw"></i> Reprocess</button>
          </div>
        </div>
      `;

      // Add reprocess listener
      const reprocessBtn = document.getElementById('btn-reprocess-purchase-pdf');
      if (reprocessBtn) {
        reprocessBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          handleReprocessPDF(purchase.pdf_url, purchase.party_inv_no, 'purchase');
        });
      }
    } else {
...
function formatToISTDateString(dateInput) {
  ...
  const dayStr = val.weekday;
  const monthStr = val.month;
  const dayNum = val.day;
  const yearStr = val.year;
  const timeStr = `${pad(val.hour)}:${pad(val.minute)}:${pad(val.second)}`;
...
async function sendSheetUpdate(type, partyInvNo, fields, record = null, options = {}) {
  ...
  // Use unique identifiers to support duplicate bill/invoice numbers (e.g. split LRs)
  let searchCol = 'party_inv_no';
  let searchVal = partyInvNo;

  if (type === 'invoice') {
    if (record && record.party_inv_no) {
      searchCol = 'party_inv_no';
      searchVal = record.party_inv_no;
    } else if (record && record.invoice_number) {
      searchCol = 'party_inv_no';
      searchVal = record.invoice_number;
    } else if (record && record.our_bill_no) {
      searchCol = 'our_bill_no';
      searchVal = record.our_bill_no;
    }
  } else if (type === 'purchase') {
    if (record && record.party_inv_no) {
      searchCol = 'party_inv_no';
      searchVal = record.party_inv_no;
    } else if (record && record.present_our_invoice && String(record.present_our_invoice).trim() !== '') {
      searchCol = 'present_our_invoice';
      searchVal = String(record.present_our_invoice).trim();
    }
  }
  ...
  try {
    const res = await safeFetch(`${SERVER_BASE_URL}/api/update-record`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    });
    ...
    try {
      const directUrl = 'https://script.google.com/macros/s/AKfycbx50k2wz5EvlNHyZELG0lRQyb7Q8m7moeaEjDSbslWo36qyW0oBVduoCRkdU21A49L5/exec';
  ...
  try {
    ...
    if (result.success) {
      ...
      if (isInvoice) {
        ...
        state.rawInvoices = (state.rawInvoices || []).filter(i => {
          const invNo = i.invoice_number || i.party_inv_no;
          return !(String(invNo) === String(invoiceNumber) && i.bill_freight_val === freight);
        });
        state.rawPurchases = (state.rawPurchases || []).filter(p => {
          const pInvNo = p.party_inv_no || p.invoice_number;
          return !(String(pInvNo) === String(invoiceNumber) && p.bill_freight_val === freight);
        });
      } else {
...
async function validateActiveBill() {
  ...
  if (invoice) {
    ...
    if (invoice._rawArray) {
      try {
        const arrObj = JSON.parse(invoice._rawArray);
        arrObj.validated = 'true';
        arrObj.validation_timestamp = nowStr;
        invoice._rawArray = JSON.stringify(arrObj);
      } catch (e) {}
    }
  }
  if (purchase) {
    ...
    if (purchase._rawArray) {
      try {
        ...
        arrObj.validated = 'true';
        arrObj.validation_timestamp = nowStr;
        if (purchase.ai_summary) arrObj.ai_summary = purchase.ai_summary;
        purchase._rawArray = JSON.stringify(arrObj);
      } catch (e) {}
    }
  }
...
async function bulkValidateSelected(type = 'invoice') {
  let selector = '.invoice-row-chk:checked';
  if (type === 'purchase') selector = '.purchase-row-chk:checked';
  if (type === 'recon') selector = '.recon-row-chk:checked';

  const checkedBoxes = document.querySelectorAll(selector);
  if (!checkedBoxes || checkedBoxes.length === 0) {
    showToast("Please select at least one record to validate.", "warning");
    return;
  }

  const idsToValidate = Array.from(checkedBoxes).map(chk => chk.dataset.id || chk.dataset.no).filter(Boolean);
  if (idsToValidate.length === 0) {
    showToast("No valid record IDs selected.", "warning");
    return;
  }
  ...
  showToast(`Validating ${idsToValidate.length} selected bills...`, "warning");
  ...
  for (const id of idsToValidate) {
    ...
    if (id.startsWith('inv-')) {
      invoice = state.invoices.find(i => i.id === id) || null;
      if (invoice) {
        invoiceNumber = invoice.invoice_number;
        const cleanNo = String(invoiceNumber).trim().toLowerCase();
        purchase = [...state.purchases].reverse().find(p => String(p.party_inv_no).trim().toLowerCase() === cleanNo && p.bill_freight_val === invoice.bill_freight_val)
                || [...state.purchases].reverse().find(p => String(p.party_inv_no).trim().toLowerCase() === cleanNo) || null;
      }
    } else if (id.startsWith('pur-')) {
      purchase = state.purchases.find(p => p.id === id) || null;
      if (purchase) {
        invoiceNumber = purchase.party_inv_no;
        const cleanNo = String(invoiceNumber).trim().toLowerCase();
        invoice = [...state.invoices].reverse().find(i => String(i.invoice_number).trim().toLowerCase() === cleanNo && i.bill_freight_val === purchase.bill_freight_val)
               || [...state.invoices].reverse().find(i => String(i.invoice_number).trim().toLowerCase() === cleanNo) || null;
      }
    } else {
      invoiceNumber = id;
      const cleanNo = String(invoiceNumber).trim().toLowerCase();
    ...
    const updatePayload = { validated: 'true', validation_timestamp: nowStr };
    const purchasePayload = {
      ...updatePayload,
      ...(purchase && purchase.ai_summary ? { ai_summary: purchase.ai_summary } : {})
    };

    if (invoice) sendSheetUpdate('invoice', invoice.invoice_number, updatePayload, invoice, { silent: true });
    if (purchase) sendSheetUpdate('purchase', purchase.party_inv_no, purchasePayload, purchase, { silent: true });
  ...
  showToast(`✓ Successfully validated ${countValidated} selected bills!`, "success");
}
window.bulkValidateSelected = bulkValidateSelected;
...
function initUploadActions() {
  ...
  // Attach Bulk Queue Control Buttons
  const nextBatchBtn = document.getElementById('btn-force-next-batch');
  const pauseQueueBtn = document.getElementById('btn-toggle-pause-queue');
  const cancelQueueBtn = document.getElementById('btn-cancel-bulk-queue');

  if (nextBatchBtn) nextBatchBtn.addEventListener('click', forceNextBatch);
  if (pauseQueueBtn) pauseQueueBtn.addEventListener('click', togglePauseBulkQueue);
  if (cancelQueueBtn) cancelQueueBtn.addEventListener('click', cancelBulkQueue);
...
// ==========================================================================
// BULK BATCH UPLOAD QUEUE ENGINE (1 PDF per Batch + Webhook Signal Wait)
// ==========================================================================
let bulkQueue = {
  files: [],
  totalFiles: 0,
...
function handleUploadFiles(fileList) {
  ...
  if (oversizedCount > 0) {
    showToast(`${oversizedCount} file(s) exceeded the 10MB limit and were skipped.`, "warning");
  }

  if (validFiles.length === 0) return;

  startBulkBatchQueue(validFiles);
}

function startBulkBatchQueue(filesList) {
  bulkQueue.files = filesList;
  bulkQueue.totalFiles = filesList.length;
  bulkQueue.batchSize = 1;
  bulkQueue.currentBatchIndex = 0;
  bulkQueue.completedCount = 0;
  bulkQueue.failedCount = 0;
  bulkQueue.isActive = true;
  bulkQueue.isPaused = false;
  bulkQueue.isWaitingForWebhook = false;
  bulkQueue.batchUploadStartTime = 0;

  showToast(`Started Bulk Queue for ${filesList.length} PDFs (1 PDF per batch).`, "info");
  
  const queueCard = document.getElementById('bulk-queue-control-card');
  if (queueCard) queueCard.classList.remove('hidden');

  updateBulkQueueUI();
  startWebhookNotificationPoller();
  executeCurrentBatch();
}

async function executeCurrentBatch() {
  if (!bulkQueue.isActive || bulkQueue.isPaused) return;

  const totalBatches = Math.ceil(bulkQueue.totalFiles / bulkQueue.batchSize);
  const batchIdx = bulkQueue.currentBatchIndex;

  if (batchIdx >= totalBatches || bulkQueue.completedCount >= bulkQueue.totalFiles) {
    completeBulkBatchQueue();
    return;
  }

  const startOffset = batchIdx * bulkQueue.batchSize;
  const batchFiles = bulkQueue.files.slice(startOffset, startOffset + bulkQueue.batchSize);
  ...
  bulkQueue.isWaitingForWebhook = false;
  bulkQueue.batchUploadStartTime = Date.now();

  updateBulkQueueUI(`Uploading PDF ${batchIdx + 1}/${totalBatches} (${batchFiles[0].name})...`);

  // Upload 1 PDF
  const uploadPromises = batchFiles.map(file => uploadSingleFileToDrive(file));

  try {
    const results = await Promise.all(uploadPromises);
    results.forEach(res => {
      if (res.success) bulkQueue.completedCount++;
      else bulkQueue.failedCount++;
    });
  } catch (err) {
    console.error("Error in batch upload:", err);
  }

  if (!bulkQueue.isActive) return;

  if (bulkQueue.completedCount + bulkQueue.failedCount >= bulkQueue.totalFiles) {
    completeBulkBatchQueue();
    return;
  }

  // PAUSE and WAIT for Webhook Signal POST (/excutedSucess, /agentError, or /notfound)
  bulkQueue.isWaitingForWebhook = true;
  bulkQueue.batchUploadStartTime = Date.now();
  updateBulkQueueUI(`PDF ${batchIdx + 1} Uploaded. Waiting for N8N webhook callback...`);

  showToast(`PDF ${batchIdx + 1} uploaded to Drive. Waiting for processing POST callback...`, "warning");
}

function uploadSingleFileToDrive(file, maxRetries = 3) {
  let attempts = 0;
  ...
  const attemptUpload = () => {
    return new Promise((resolve) => {
      ...
      const xhr = new XMLHttpRequest();
      xhr.open('POST', `${SERVER_BASE_URL}/upload`, true);
      ...
      xhr.addEventListener('load', async () => {
        try {
          ...
          if (data.success) {
            ...
            if (attempts < maxRetries && bulkQueue.isActive) {
              console.warn(`Retry upload attempt ${attempts}/${maxRetries} for ${file.name}`);
              setTimeout(async () => {
                const retryRes = await attemptUpload();
                resolve(retryRes);
              }, 2000);
            } else {
              showToast(`Upload failed for ${file.name}: ${data.error || 'unknown error'}`, 'error');
      ...
      xhr.addEventListener('error', async () => {
        if (attempts < maxRetries && bulkQueue.isActive) {
          ...
          showToast(`Upload failed for ${file.name} — server unreachable`, 'error');
          addUploadLog(file.name, 'error');
          resolve({ success: false, file, error: 'network_error' });
        }
      });
...
// Web Worker for Background Tab Resilience (Exempt from Browser Timer Throttling)
let bgWorker = null;

function initBackgroundQueueWorker() {
  ...
  const workerCode = `
    ...
    self.onmessage = function(e) {
      ...
      if (action === 'START') {
        ...
        batchStartTime = payload.batchStartTime;
        lastKnownId = payload.lastKnownId;

        if (pollingInterval) clearInterval(pollingInterval);
        pollingInterval = setInterval(checkNotification, 1200);
      } else if (action === 'STOP') {
        if (pollingInterval) clearInterval(pollingInterval);
    ...
    async function checkNotification() {
      try {
        const res = await fetch(baseUrl + '/api/agent-notification?_=' + Date.now(), {
        ...
        if (data && data.success && data.notification) {
          ...
          if (notif.id !== lastKnownId && notif.timestamp >= batchStartTime) {
            lastKnownId = notif.id;
            self.postMessage({ type: 'WEBHOOK_SIGNAL', notification: notif });
          }
        }
      } catch (err) {
  ...
  try {
    ...
    bgWorker.onmessage = (e) => {
      if (e.data && e.data.type === 'WEBHOOK_SIGNAL') {
        handleWebhookSignalReceived(e.data.notification);
      }
    };
  } catch (err) {
    console.warn("Web Worker fallback to main thread poller:", err.message);
  }
  return bgWorker;
}

function handleWebhookSignalReceived(notif) {
  if (!bulkQueue.isActive) return;
  if (notif.id === bulkQueue.lastKnownNotificationId) return;

  bulkQueue.lastKnownNotificationId = notif.id;
  console.log("Received new webhook signal during bulk queue wait:", notif);

  const msgEl = document.getElementById('bulk-queue-webhook-message');
  if (msgEl) {
    msgEl.textContent = `Signal Received [${notif.endpoint || ''}]: ${notif.message}`;
  }

  if (bulkQueue.isWaitingForWebhook && !bulkQueue.isPaused) {
    showToast(`Webhook signal received (${notif.endpoint || 'N8N'})! Resuming next PDF...`, "success");
    bulkQueue.isWaitingForWebhook = false;
    bulkQueue.currentBatchIndex++;
...
function startWebhookNotificationPoller() {
  const worker = initBackgroundQueueWorker();
  if (worker) {
    worker.postMessage({
      action: 'START',
      payload: {
        baseUrl: SERVER_BASE_URL,
        batchStartTime: bulkQueue.batchUploadStartTime,
        lastKnownId: bulkQueue.lastKnownNotificationId
      }
    });
  }

  if (bulkQueue.pollingIntervalId) clearInterval(bulkQueue.pollingIntervalId);

  bulkQueue.pollingIntervalId = setInterval(async () => {
    if (!bulkQueue.isActive) {
      clearInterval(bulkQueue.pollingIntervalId);
      bulkQueue.pollingIntervalId = null;
      if (worker) worker.postMessage({ action: 'STOP' });
      return;
    }

    try {
      const res = await safeFetch(`${SERVER_BASE_URL}/api/agent-notification?_=${Date.now()}`);
      if (!res.ok) return;
      const data = await res.json();
      
      if (data.success && data.notification) {
        handleWebhookSignalReceived(data.notification);
      }
    } catch (e) {
...
// Keep UI fresh when switching back from other browser tabs
document.addEventListener('visibilitychange', () => {
  if (!document.hidden && bulkQueue.isActive) {
    updateBulkQueueUI();
    if (typeof lucide !== 'undefined') lucide.createIcons();
  }
});
...
function forceNextBatch() {
  if (!bulkQueue.isActive) return;
  showToast("Manually forcing next batch...", "info");
  bulkQueue.isWaitingForWebhook = false;
  bulkQueue.currentBatchIndex++;
  executeCurrentBatch();
}

function togglePauseBulkQueue() {
  bulkQueue.isPaused = !bulkQueue.isPaused;
  const pauseBtn = document.getElementById('btn-toggle-pause-queue');
  if (pauseBtn) {
    pauseBtn.innerHTML = bulkQueue.isPaused ? '<i data-lucide="play"></i> Resume' : '<i data-lucide="pause"></i> Pause';
    if (typeof lucide !== 'undefined') lucide.createIcons();
  }

  if (bulkQueue.isPaused) {
    showToast("Bulk queue paused.", "warning");
    updateBulkQueueUI("Queue Paused by User");
  } else {
    showToast("Resuming bulk queue...", "success");
    if (!bulkQueue.isWaitingForWebhook) {
      executeCurrentBatch();
    }
  }
}
...
function cancelBulkQueue() {
  bulkQueue.isActive = false;
  bulkQueue.isWaitingForWebhook = false;
  if (bulkQueue.pollingIntervalId) {
    clearInterval(bulkQueue.pollingIntervalId);
    bulkQueue.pollingIntervalId = null;
  }
  const queueCard = document.getElementById('bulk-queue-control-card');
  if (queueCard) queueCard.classList.add('hidden');
  showToast("Bulk queue cancelled.", "warning");
}

function completeBulkBatchQueue() {
  bulkQueue.isActive = false;
  bulkQueue.isWaitingForWebhook = false;
  if (bulkQueue.pollingIntervalId) {
    clearInterval(bulkQueue.pollingIntervalId);
  ...
  updateBulkQueueUI(`Bulk Queue Complete! ${bulkQueue.completedCount} / ${bulkQueue.totalFiles} PDFs processed.`);
  showToast(`🎉 Bulk Batch Upload Complete! ${bulkQueue.completedCount} PDFs uploaded & verified.`, "success");

  syncWithAPI(true);
}

function updateBulkQueueUI(customStatus) {
  const totalBatches = Math.ceil(bulkQueue.totalFiles / (bulkQueue.batchSize || 2)) || 1;
  const currentBatch = Math.min(bulkQueue.currentBatchIndex + 1, totalBatches);
  const pct = Math.round((bulkQueue.completedCount / bulkQueue.totalFiles) * 100) || 0;

  const badge = document.getElementById('bulk-queue-badge');
  const counterText = document.getElementById('bulk-queue-counter-text');
  const statusText = document.getElementById('bulk-queue-status-text');
  const percentText = document.getElementById('bulk-queue-percent-text');
  const progressBar = document.getElementById('bulk-queue-progress-bar');
  const webhookMsg = document.getElementById('bulk-queue-webhook-message');

  if (badge) badge.textContent = `PDF ${currentBatch} / ${totalBatches}`;
  if (counterText) counterText.textContent = `${bulkQueue.completedCount} / ${bulkQueue.totalFiles} PDFs Processed`;
  ...
  if (statusText) {
    ...
    } else if (bulkQueue.isWaitingForWebhook) {
      statusText.textContent = `PDF ${currentBatch} Uploaded. Waiting for N8N POST Callback...`;
    } else {
      statusText.textContent = `Processing PDF ${currentBatch} of ${totalBatches}...`;
    }
  }

  if (webhookMsg && !customStatus) {
    if (bulkQueue.isWaitingForWebhook) {
      webhookMsg.textContent = `Waiting for N8N webhook POST (/excutedSucess, /agentError, or /notfound)...`;
    }
  }
}
...
function initModalActions() {
  ...
  document.getElementById('review-bill-btn').addEventListener('click', reviewActiveBill);

  document.getElementById('validate-bill-btn').addEventListener('click', validateActiveBill);

  const invValBtn = document.getElementById('inv-validate-selected-btn');
  if (invValBtn) invValBtn.addEventListener('click', () => bulkValidateSelected('invoice'));

  const purValBtn = document.getElementById('pur-validate-selected-btn');
  if (purValBtn) purValBtn.addEventListener('click', () => bulkValidateSelected('purchase'));

  const reconValBtn = document.getElementById('recon-validate-selected-btn');
  if (reconValBtn) reconValBtn.addEventListener('click', () => bulkValidateSelected('recon'));
...
function renderRichAiSummary(rawSummary) {
  ...
  const devReviewIdx = lowerSummary.indexOf('dev review need');
  ...
  if (passedIdx !== -1) {
    const nextIdx = warningsIdx !== -1 ? warningsIdx : (failedIdx !== -1 ? failedIdx : (devReviewIdx !== -1 ? devReviewIdx : -1));
    passedText = extractSection(passedIdx, nextIdx);
  }
  if (warningsIdx !== -1) {
    const nextIdx = failedIdx !== -1 ? failedIdx : (devReviewIdx !== -1 ? devReviewIdx : -1);
    warningsText = extractSection(warningsIdx, nextIdx);
  }
  if (failedIdx !== -1) {
    const nextIdx = devReviewIdx !== -1 ? devReviewIdx : -1;
    failedText = extractSection(failedIdx, nextIdx);
  }

  let devReviewRaw = '';
  if (devReviewIdx !== -1) {
    const endIdx = lowerSummary.indexOf('====', devReviewIdx);
    if (endIdx !== -1) {
      devReviewRaw = cleanedSummary.substring(devReviewIdx, endIdx).trim();
    } else {
      devReviewRaw = cleanedSummary.substring(devReviewIdx).trim();
    }
  }

  if (passedIdx === -1 && warningsIdx === -1 && failedIdx === -1 && devReviewIdx === -1) {
  ...
  if (devReviewRaw) {
    const remarkMatch = devReviewRaw.match(/dev\s+review\s+need\s*:\s*([\s\S]+)/i);
    const remarkContent = remarkMatch ? remarkMatch[1].trim() : 'Dev Review Needed';
    html += `
      <div class="ai-section dev-review" style="background-color: rgba(255, 193, 7, 0.15); border: 2px solid #ffc107; border-left: 8px solid #ffc107; padding: 16px; border-radius: 8px; margin-top: 10px;">
        <span style="font-weight: 800; color: #d39e00; display: block; margin-bottom: 6px; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.75px;">⚠️ Dev Review Requested:</span>
        <div style="color: var(--text-main); font-size: 0.95rem; font-weight: 600; line-height: 1.5;">${escapeHtml(remarkContent)}</div>
      </div>
    `;
  }
  ...
  if (settingsBtn) {
    settingsBtn.addEventListener('click', () => {
      if (getCurrentUserRole() !== 'admin') {
        showToast('Settings section is restricted to Admin users only.', 'warning');
        return;
      }
